...
def test():
    ...
    df_test = preprocess(trainer.DATA_DIR,csv_dir=path_csv_test,train_val_split_status=False)
    ...
    test_dataloader = DataLoader(
                test_dataset,
                batch_size=round(len(data_val)), #FIXME:
                shuffle=True)
    ...
    trainer.epoch_evaluate(test_dataloader, model, loss_fn, device, use_checkpoint = False)
